...
public class DialogBox extends JDialog {
	...
	public void chooseFeatures() {
		Listener_Mouse lm = new Listener_Mouse();

		ij = new JRadioButton("ImageJ Features", true);
		ij.setActionCommand("IJ");
		ij.addActionListener(lm);
		jf = new JRadioButton("JFeatureLib Features", false);
		jf.setActionCommand("JF");
		jf.addActionListener(lm);
		
		JPanel texture = new JPanel();
		texture.add(ij);
		texture.add(jf);
		
		JOptionPane.showInternalMessageDialog(comp, texture, "Choose which features to use:", JOptionPane.QUESTION_MESSAGE);
	}
